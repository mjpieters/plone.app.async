...
Make sure that the dispatcher sees the same db:
    ...
    >>> job = async.queueJob(dbUsed, self.folder)
    >>> transaction.commit()
...
In real life the job would be done. In the tests we need to commit in order to
let the  dispatcher become aware of the job and execute it. Also we wait for
the job to complete before continuing with the test.
...
job that submits the document implies that the one that creates has already 
been carried out.
...
If you need to act on the result of a job or you handle a failure you can do
so by adding callbacks. For example:
...
want to send an email upon failures) by subscribing to the relevant events:
